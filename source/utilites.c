...
void print_coming_notes(int position) {
    ...
    int8_t max, cur, max_h, temp[10];

    printf("\n");
    for(max_h = 0, i = position; i < end; i++)
        for(j = 1; j <= tune_data[i][0] / 0x10; j++)
            if(max_h == 0) temp[max_h++] = tune_data[i][j]&0b01111111;
            else {
                for(k = 0; k < max_h; k++) if((tune_data[i][j]&0b01111111) == temp[k]) break;
                if(k == max_h) temp[max_h++] = tune_data[i][j]&0b01111111;
            }
    for(max = k = 0, cur = 127; k < max_h; k++){
        for(i = position; i < end; i++)
            for(j = 1; j <= tune_data[i][0]/0x10; j++)
                if((tune_data[i][j]&0b01111111) > max && (tune_data[i][j]&0b01111111) < cur)
                    max = tune_data[i][j]&0b01111111;
        for(i = position; i < end; i++) {
            *((int32_t*)note) = *((int32_t*)"|  ");
            for(j = 1; j <= tune_data[i][0]/0x10; j++)
                if((tune_data[i][j]&0b01111111) == max) {
                    note_from_midi_code(max, note);
                    if(tune_data[i][j] < 0b10000000) note[0] += 'a' - 'A';
                    break;
                }
            printf("%-4s", note);
        }
        cur = max; max = 0; printf("\n");
    }
